...
def weight_map(maskfile, science_image): #Taken from Create_Weight_Map.py
	...
	'''
	try:
		os.remove('Weight_Map/'+science_image[1]+'_Weight_Map.fits') #removes a file if it has the same name because astropy doesn't like overwriting existing files
		#print 'Old ', science_image, '_Weight_map.fits was removed, new file will be written'
	except OSError:
		pass
	W_DEST=str('Weight_Map/'+science_image[1]+'_Weight_Map.fits')	
	print 'W_DEST: ', W_DEST
	...
	hdulist_mask.writeto('Weight_Map/'+science_image[1]+'_Weight_Map.fits') #New fits file. Its the WEIGHT_IMAGE
...
def Sextract(science_image,zeropoint,seeing,saturation,gain): #Taken from Create_sexed_catalog.py
	...
	subprocess.call('sex -c Pipe_sexfile_CMD.sex '+science_image[0]+science_image[1]+'.fits -PARAMETERS_NAME PTF_Transform_Param.param -FILTER_NAME default.conv -CATALOG_NAME Catalog/'+science_image[1]+'_Catalog.cat -WEIGHT_IMAGE Weight_Map/'+science_image[1]+'_Weight_map.fits -MAG_ZEROPOINT'+'	'+str(zeropoint)+' -SEEING_FWHM '+str(seeing)+' -SATUR_LEVEL '+str(saturation)+' -GAIN '+str(gain)+' -VERBOSE_TYPE NORMAL',shell=True)

def Enough_Objects(science_image):
	enough=True
	test=os.popen('wc -l Catalog/'+science_image[1]+'_Catalog.cat').read()
	rows=test.split()
	if float(rows[0])<217:
			return False
...
def Selecting_Bright(science_image):
	...
	f=open('Catalog/'+science_image[1]+'_Catalog.cat') #CHANGE THIS SO THAT IT CAN TAKE ANY CATALOG INPUT
	fin=f.readline()
...
def selecting_galaxies(science_image,):
	f=open('Catalog/'+science_image[1]+'_Catalog.cat')
	g=open('Galaxies/'+science_image[1]+'_Galaxy_Catalog.cat','w')
	l=open('Galaxies/'+science_image[1]+'_Galaxy_regions.reg','w')
	m=open('Galaxies/'+science_image[1]+'_Star_regions.reg','w')
	fin=f.readline()
	X2_array=[]
	Y2_array=[]
	FWHM_array=[]
...
def add_fakes_2galaxy(science_image,boxsize, xcord_star, ycord_star, newx_star, newy_star, mag_array_star, flux_array_star, ran_mag_star, ran_flux_star, background_array_star, scaling_factor_star, CCD_Num_star):
	...
	h=open('Galaxies/'+science_image[1]+'_Galaxy_Catalog.cat') #Opens the Galaxy catalog
	f=open('Fake_Star_Catalog/'+science_image[1]+'_Fake_Star_Catalog.dat','w') #Opens the fake star catalog
	reg=open('Fake_Star_Catalog/'+science_image[1]+'_Fakes_Star_Regions.reg','w') #creates region file
	...
	j=open('Fakes/'+science_image[1]+'_Flux_Boxes.dat','w')
	for i in range(0,len(xcord_star)): #Will only add 200 fake stars to 200 Galaxies
	...
	try:
		os.remove('Fakes/'+science_image[1]+'_WITH_FAKES.fits') #removes a file if it has the same name because astropy doesn't like overwriting existing files
		#print 'Old Fake Removed'
	except OSError:
		pass
		
	hdulist_sci.writeto('Fakes/'+science_image[1]+'_WITH_FAKES.fits') #Saving image after loop of 200 Stars is complete
	j.close()
	reg.close()	
	f.close()
	print 'Fake Stars Added to Galaxies in the Image: ', science_image[1]
...
def Execute(run):
	...
	saturation=float(hdulist_multi_sci[0].header['SATURATE'])
	...
	if catsize==False:
			...
			os.remove('Catalog/'+science_image[1]+'_Catalog.cat')
			return
...
#path=[]
#fnames=[]
for dirpath,dirnames,filenames in os.walk(os.path.abspath('../refs')): #Traverses through a directory tree from 'refs' (refs is a folder in your CWD)
...
for i in range(len(all_fits)):
	...
	if fname[-2]=='noise' or fname[-2]=='weight':
		pass
	else:
		#print ln[1]
		#print all_fits[i]
...
bad_images=open('Bad_Images.dat','w')
bad_images.close()
